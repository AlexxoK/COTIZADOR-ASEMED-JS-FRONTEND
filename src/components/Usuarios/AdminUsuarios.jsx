...
const AdminUsuarios = () => {
    ...
    const { listaUsuarios, handleListaUsuarios, handleActualizarUsuario, handleEliminarUsuario, loading } = useClienteHook();
    ...
    return (
        <div>
            ...
            <main className="adm-main">
                ...
                {admins.length > 0 && (
                    <>
                        <h2 className="adm-section-title"> <FaUserShield /> Administradores</h2>
                                ...
                                <li key={usuario.uid} className="adm-user-card admin">
                                    ...
                                    <div className="adm-user-info">
                                        ...
                                        <div className="adm-user-actions">
                                            <button className="btn-role-admin" onClick={() => toggleRole(usuario)}>Cambiar Role</button>
                                            <button className="btn-delete" onClick={() => handleEliminarUsuario(usuario.uid)}>Eliminar Usuario</button>
                                        </div>
                                    </div>
                                </li>
                            ))}
                ...
                {clientes.length > 0 && (
                    <>
                        <h2 className="adm-section-title"> <FaUser /> Clientes</h2>
                        <ul className="adm-user-list">
                            {clientes.map(usuario => (
                                <li key={usuario.uid} className="adm-user-card cliente">
                                    ...
                                    <div className="adm-user-info">
                                        ...
                                        <p>UID: {usuario.uid}</p>
                                        <div className="adm-user-actions">
                                            <button className="btn-role-cliente" onClick={() => toggleRole(usuario)}>Cambiar Role</button>
                                            <button className="btn-delete" onClick={() => handleEliminarUsuario(usuario.uid)}>Eliminar Usuario</button>
                                        </div>
                                    </div>
                                </li>
